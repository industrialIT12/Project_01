...
const Mastertable = ({project_id, onExportData}) => {
    const [cust_name, setcust_name] = useState('');
    const [cust_category, setcust_category] = useState('');
  ...
  const handleSubmit2 = async () => {
    try {
      ...
      const response = await fetch('http://163.125.102.142:6500/api/addmaster', {
        method: 'POST',
        headers: {
  ...
  const columns = [
    ...
    { field: "customer_category", headerName: "Customer Category", flex: 1 },
  ...
  return (
    ...
    <Button variant="contained" color="secondary" onClick={handleOpen2}> Add Master Details</Button>
  </Box>
...
<TextField
  fullWidth
  value={cust_category}
  onChange={(e) => setcust_category(e.target.value)}
  sx={{ mt: 2 }}
