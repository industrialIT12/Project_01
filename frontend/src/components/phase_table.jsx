...
const CustomerPhaseTable = ({ selectedBusiness }) => {
  ...
  useEffect(() => {
    ...
    fetch(`http://163.125.102.142:6500/details?business=${selectedBusiness.join(",")}`)
      .then(response => {
        if (!response.ok) {
...
>
  ...
  {selectedProject ? (
    ...
    <Box overflowX="auto" borderRadius='40px' boxShadow="10px">
      <Table
        sx={{
          minWidth: 650,
